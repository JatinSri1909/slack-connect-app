...
import type { IScheduledMessage, IScheduleMessageRequest, ISendMessageRequest, ISlackChannel } from '@/types';
...
class ApiService {
  ...
  async sendMessage(data: ISendMessageRequest): Promise<{ success: boolean; message: string }> {
    const response = await api.post('/slack/send-message', data);
    return response.data;
  }

  async scheduleMessage(data: IScheduleMessageRequest): Promise<{ success: boolean; messageId: number; message: string }> {
    const response = await api.post('/slack/schedule-message', data);
    return response.data;
  }

  async getScheduledMessages(teamId: string): Promise<{ messages: IScheduledMessage[] }> {
